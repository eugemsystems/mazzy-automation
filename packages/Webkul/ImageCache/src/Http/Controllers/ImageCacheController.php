...
class ImageCacheController extends Controller
{
    ...
    protected function getImagePath(string $filename): string
    {
        ...
        foreach ($paths as $basePath) {
            ...
            if ($realPath && str_starts_with($normalize($realPath), $normalize($basePath).'/')) {
                return $realPath;
            }
        }
        ...
        if ($storageBase) {
            ...
            if ($realPath && str_starts_with($normalize($realPath), $normalize($storageBase).'/')) {
                return $realPath;
            }
        }
        ...
        if ($publicBase) {
            ...
            if ($realPath && str_starts_with($normalize($realPath), $normalize($publicBase).'/')) {
                return $realPath;
            }
        }
        ...
        if ($storagePublicBase) {
            ...
            if ($realPath && str_starts_with($normalize($realPath), $normalize($storagePublicBase).'/')) {
                return $realPath;
            }
        }

        return storage_path('app/public'.DIRECTORY_SEPARATOR.$filename);
    }
