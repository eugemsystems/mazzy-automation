    ...
    <div style="background:#332a5e; padding:28px 0 22px;">
        <div class="container">
            <h1 style="color:#fff; font-size:22px; font-weight:700; margin:0 0 6px;">
                @if(!empty($query)) Search: {{ $query }} @else All Products @endif
            </h1>
            <nav style="font-size:13px;">
                ...
                @if(!empty($query))
                    <span style="color:rgba(255,255,255,.4); margin:0 8px;">/</span>
                    <span style="color:#FF9923;">Search</span>
                @endif
            </nav>
        </div>
    </div>
    ...
    <div class="container py-4">
        ...
        @if ($searchInstead)
            <form
                ...
                role="search"
            >
                <input type="text" name="query" style="display:none;" value="{{ $searchInstead }}">
                <input type="text" name="suggest" style="display:none;" value="0">
                <p style="margin:0; color:#6c757d; font-size:13px;" v-pre>
                    {{ trans('shop::app.search.suggest') }}
                    <button type="submit" style="background:none; border:none; color:#332a5e; text-decoration:underline; cursor:pointer; font-size:inherit; padding:0;">{{ $searchInstead }}</button>
                </p>
            </form>
        @endif
    </div>
