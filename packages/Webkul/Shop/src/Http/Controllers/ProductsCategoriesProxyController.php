...
class ProductsCategoriesProxyController extends Controller
{
    ...
    {
        // Use the route parameter directly to avoid including the route prefix (e.g. /store/)
        $slugOrURLKey = urldecode(trim($request->route('path') ?? $request->getPathInfo(), '/'));

        // Strip any leading route prefix segments (e.g. 'store/') that getPathInfo may include
        $routePrefix = trim($request->route()->getPrefix() ?? '', '/');
        if ($routePrefix && str_starts_with($slugOrURLKey, $routePrefix.'/')) {
            $slugOrURLKey = substr($slugOrURLKey, strlen($routePrefix) + 1);
        }
